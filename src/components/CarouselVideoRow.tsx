          ...
          {videos.map((video) => {
            ...
            return (
              <SwiperSlide key={video.id} className="overflow-visible">
                <SwiperSlideContent video={video} youtubeId={id} onSelect={onSelect} />
              </SwiperSlide>
            );
          })}
        </Swiper>
...
// ✅ Extracted child component to safely use useState
function SwiperSlideContent({
  ...
  const thumbnail =
    video.thumbnail?.url || `https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`;
